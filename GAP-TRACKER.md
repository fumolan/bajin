...
- [ ] **SQLite 会话库**（2026-08-16 夜间完成 rewind 切片）：`--rewind <n>` 已完成——`core/session.ts rewindTranscript`（一轮=user 消息块，从尾部裁原始行不重序列化，损坏行不误判轮起点，n 超界清空、n<=0 no-op）+ CLI `--rewind <n>`（配合 --resume/-c，校验正整数，仅裁 transcript 不动 meta），rewind.test.ts 4 项；**剩余**：node:sqlite 替代 JSONL transcript（表：session/message/part/todo/tool_usage），保持 loadTranscript/listSessions API 兼容；迁移已有 JSONL
